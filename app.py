...
def _read_uploaded_bytes(file_bytes, filename, sheet_name=None):
...
@st.cache_data(show_spinner=False)
def _excel_sheet_names_from_bytes(file_bytes, filename):
    lower = filename.lower()
...
if page == "Executive Dashboard":
    ...
    st.markdown("### Understanding the Fragility Level")
    st.markdown(
        "SCFI is a continuous measure. The classes below provide a simple warning layer so a first-time user can understand what the current value means and how the levels are differentiated."
    )

    fc1, fc2, fc3 = st.columns(3)
    with fc1:
        st.markdown(
            '<div class="risk-moderate"><strong>MODERATE - SCFI &lt; 0.5</strong><br>'
            'Conditions are closer to normal historical variability. Individual pressures may exist, but the combined operational, macroeconomic, geopolitical and energy-market signal is not strongly elevated.</div>',
            unsafe_allow_html=True
        )
    with fc2:
        st.markdown(
            '<div class="risk-high"><strong>HIGH - 0.5 ≤ SCFI &lt; 1.5</strong><br>'
            'Fragility is elevated. Multiple pressure dimensions may be contributing and the environment should be monitored more closely for further deterioration.</div>',
            unsafe_allow_html=True
        )
    with fc3:
        st.markdown(
            '<div class="risk-severe"><strong>SEVERE - SCFI ≥ 1.5</strong><br>'
            'Fragility is unusually broad and intense. Several pressure dimensions are reinforcing each other strongly enough to indicate a severe warning condition.</div>',
            unsafe_allow_html=True
        )

    st.markdown(
        risk_html(
            current["Fragility_Class"],
            f"Current interpretation: SCFI {current_scfi:.4f} is {current['Fragility_Class'].upper()}. "
            f"The {selected_model} next-month forecast is {selected_forecast['Forecast']:.4f}, "
            f"classified as {selected_forecast['Fragility'].upper()}. "
            "Read the continuous value together with the class because it also shows how close the result is to the next threshold."
        ),
        unsafe_allow_html=True
    )
