...
def create_excel_report(results, selected_model, scenario_table=None):
    ...
    with pd.ExcelWriter(output, engine="openpyxl") as writer:
        results["quality"].to_excel(writer, "Data Quality", index=False)
        master.to_excel(writer, "Merged Data", index=False)
        scaled.to_excel(writer, "SCFI History", index=False)
        results["pca_table"].to_excel(writer, "PCA Weights", index=False)
        validation.to_excel(writer, "Validation", index=False)
        results["model_comparison"].to_excel(
            writer, "Regression Models", index=False
        )
        forecast.to_excel(writer, "Selected Forecast", index=False)
...
with pages[4]:
    ...
    forecast_fig.add_hline(y=.5, line_dash="dot")
    forecast_fig.add_hline(y=1.5, line_dash="dot")
